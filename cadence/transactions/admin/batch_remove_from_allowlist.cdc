...
/// @title Batch Remove from Allowlist
/// @notice Removes multiple addresses from the allowlist on the EVM FlowYieldVaultsRequests contract
...
///
/// @param addresses Array of EVM address strings to remove from the allowlist
///
transaction(addresses: [String]) {
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        worker.batchRemoveFromAllowlist(evmAddresses)
    }
}
