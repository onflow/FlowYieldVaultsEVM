...
/// @notice Removes multiple addresses from the blocklist on the EVM FlowYieldVaultsRequests contract
...
///
/// @param addresses Array of EVM address strings to remove from the blocklist
///
transaction(addresses: [String]) {
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        worker.batchRemoveFromBlocklist(evmAddresses)
    }
}
