...
contract FlowYieldVaultsRequests is ReentrancyGuard, Ownable2Step {
    ...
    function _completeProcessing(
        ...
        if (!success) {
            ...
            uint256 expectedSuccessRefundAmount = isEscrowedRequest
                ? _fyvAdmin.expectedPrecisionResidual(
                    request.tokenAddress,
                    request.amount
                )
                : 0;
    ...
    function _unregisterYieldVault(uint64 yieldVaultId, address user, uint256 requestId) internal {
        ...
        for (uint i = 0; i < userYieldVaults.length - 1;) {
            if (userYieldVaults[i] == yieldVaultId) {
                userYieldVaults[i] = userYieldVaults[i + 1];
            }
            unchecked {
                ++i;
            }
    ...
    function _removePendingRequest(uint256 requestId) internal {
        ...
        uint256[] storage userPendingIds = pendingRequestIdsByUser[user];
        for (uint i = 0; i < userPendingIds.length - 1;) {
            if (userPendingIds[i] == requestId) {
                userPendingIds[i] = userPendingIds[i + 1];
            }
            unchecked {
                ++i;
            }
